  ...
  if (fields.ContentProducer && fields.ContentProducer.length >= 4) {
    ...
    producerHtml = `
      <div class="mt-2 rounded-lg bg-surface/50 p-3 text-xs font-mono space-y-1">
        ${p.knownName ? `<div><span class="text-text-muted">机构名称：</span><span class="text-accent-cyan">${p.knownName}</span></div>` : ''}
        <div><span class="text-text-muted">主体类型：</span><span class="text-text-secondary">${p.entityType}</span></div>
        <div><span class="text-text-muted">统一信用代码：</span><span class="text-text-primary">${p.creditCode}</span></div>
        ${p.serviceCode !== '—' ? `<div><span class="text-text-muted">业务码：</span><span class="text-text-secondary">${p.serviceCode}</span></div>` : ''}
      </div>`
  }
    ...
    .map(([key, cn]) => {
      ...
      return `
        <div class="flex gap-2 py-1.5 border-b border-surface/60 last:border-0 text-xs">
          ...
          <span class="text-text-primary font-mono break-all">${val === '—' ? '<span class="text-text-muted">—</span>' : val}</span>
        </div>
  ...
  return `
    <div class="rounded-xl border border-${statusColor}/30 bg-card aigc-glow p-4 mb-4">
      <div class="flex items-center gap-2 mb-3">
        ...
        <span class="ml-auto text-text-muted text-xs">${source}</span>
      </div>
